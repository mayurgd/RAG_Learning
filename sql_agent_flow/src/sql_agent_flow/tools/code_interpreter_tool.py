...
def get_last_assigned_variable(code_str):
    try:
        # Parse the code into an AST (Abstract Syntax Tree)
        tree = ast.parse(code_str)

        # Filter out assignment nodes
        assignments = [node for node in ast.walk(tree) if isinstance(node, ast.Assign)]

        if not assignments:
            return None  # No assignments found

        # Get the last assignment node
        last_assignment = assignments[-1]

        # Get the variable name (assuming simple Name assignment, not tuple/list unpacking)
        if isinstance(last_assignment.targets[0], ast.Name):
            return last_assignment.targets[0].id
        else:
...
class CodeInterpreterTool(BaseTool):
    ...
    def run_code_unsafe(self, code: str, libraries_used: List[str]) -> str:
        ...
        try:
            ...
            print(get_last_assigned_variable(code))
            return exec_locals.get(
                get_last_assigned_variable(code), "No result variable found."
            )
        except Exception as e:
